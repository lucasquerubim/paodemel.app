 ...
 * Padaria Pão de Mel
 *
 * Navegação entre páginas, utilitários globais e inicialização.
 */
...
const App = (() => {
  ...
  function goTo(page, btn) {
    ...
    if (btn) {
      ...
      // Encontra o botão correto pelo data-page
  ...
  function init() {
    ...
    // Solicita permissão de notificação ao primeiro clique
  ...
  return {
    // Constantes (acessíveis pelos outros módulos)
    STATUS_LABELS,
    STATUS_ICONS,
    TIPO_LABELS,
    EMAILS,
...
// ── Dashboard (separado mas sem arquivo próprio por ser simples) ──
...
const Dashboard = (() => {
  ...
  function render() {
    ...
    const todayOrders = pedidos.filter(p =>
      p.dtEntrega === hoje || p.dtCriado === hoje
    );
